...
Currently Aadi Panchang Map's data is bundled as binary protocol buffer files containing stars,
...
problems:
   ...
   1. The build process is awful, largely due to the way localization is done. Typically in
    ...
    these ids are generated and are not known in advance. Consequently Aadi Panchang Map uses a convoluted
...
Key features to preserve in Aadi Panchang Map are its small footprint and ability to work offline where
